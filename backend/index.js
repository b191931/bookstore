...
app.get("/homebooks", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ status: false, message: "Error fetching matching images" });
  }
});
  ...
  try {
    ...
    if (user.password === req.body.currentPassword) {
      if (user.password === req.body.newPassword) {
        res
          .status(400)
          .json({ status: false, message: "Please do not enter old password" });
  ...
  try {
    ...
    if (existUser) {
      if (req.body.password === existUser.password) {
        ...
        return res
          .status(200)
          .json({ status: true, data: { existUser, authToken } });
      } else {
...
app.get("/productpage/:token", async (req, res) => {
  try {
    ...
    const allSoldImages = allUsers.reduce((soldImages, user) => {
      ...
      const filteredSoldBooks = user.soldBooks.filter(
        (book) => book.buyer === ""
      );
    ...
    res
      .status(500)
      .json({ status: false, message: "Error fetching sold images" });
  }
});
...
app.post("/send-email", async (req, res) => {
  ...
  try {
    const userWithImage = await User.findOne({
      "soldBooks.imageName": bookName,
    });
    if (!userWithImage) {
      return res.status(404).json({
        status: false,
        message: "User with the provided image not found",
      });
    }
    ...
    const transporter = nodemailer.createTransport({
      service: "Gmail",
      auth: {
        user: "bookstore748@gmail.com", // Replace with your Gmail email
        pass: "heai gzzf dncm usry", // Replace with your Gmail password
      },
    });
    const mailOptions = {
      from: "bookstore748@gmail.com",
    ...
    transporter.sendMail(mailOptions, (error, info) => {
      if (error) {
        ...
        return res.status(200).json({
          status: true,
          message: `Email sent to ${sellerUserEmail} regarding book: ${bookName}`,
        });
      }
    });
  } catch (error) {
...
app.post("/uploadProfilePhoto", profilePhotoUpload, async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ status: true, message: "Profile photo uploaded successfully" });
  } catch (error) {
    console.error("Error uploading profile photo:", error);
    res
      .status(500)
      .json({ status: false, message: "Error uploading profile photo" });
  }
});
...
app.post("/send-verification-otp", async (req, res) => {
  try {
    ...
    if (existingOTP && existingOTP.expiry > Date.now()) {
      ...
      try {
        return res.status(400).json({
          status: false,
          message:
            "OTP has already been sent. Please wait before requesting another.",
        });
      } catch (e) {
        console.log(e);
      }
    ...
    const mailOptions = {
      from: "bookstore748@gmail.com", // Replace with your Gmail email
      to: email,
      subject: "Book Purchase Inquiry",
    ...
    transporter.sendMail(mailOptions, (error, info) => {
      if (error) {
        ...
        res
          .status(200)
          .json({ status: true, message: `otp sent to  the mail ${email}` });
      }
    });
  } catch (error) {
    ...
    res
      .status(500)
      .json({ status: false, message: "Error sending verification OTP" });
  }
});
...
app.post("/verify-email-otp", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ status: false, message: "Error verifying email OTP" });
  }
});
